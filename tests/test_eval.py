...
from app.labels import BangNhan, Label
...
def _lo(task="KIS", dong=(), qid="q1") -> LoNop:
    return LoNop(query_id=qid, task_type=task, answers=tuple(dong))


def _kis(video="L01_V001", frame=505) -> DongNop:
    return DongNop(video_id=video, frame_ids=(frame,))
...
def test_kis_dung_frame_trong_khoang():
    bn = BangNhan([_nhan()])
    kq = cham_query(_lo("KIS", [_kis(frame=505)]), bn)
...
def test_kis_frame_ngoai_khoang_thi_0():
    ...
    bn = BangNhan([_nhan()])
    assert cham_query(_lo("KIS", [_kis(frame=600)]), bn).r_scores[0] == 0.0


def test_kis_SAI_VIDEO_thi_0_du_frame_dung():
    """Ví dụ BTC: nộp L02_V003, 505 → sai video → 0."""
    bn = BangNhan([_nhan()])
    assert cham_query(_lo("KIS", [_kis(video="L02_V003", frame=505)]), bn).r_scores[0] == 0.0


def test_kis_hang_dau_tien_va_final_khop_nhau():
    bn = BangNhan([_nhan()])
    dong = [_kis(frame=600), _kis(frame=600), _kis(frame=505)]
    kq = cham_query(_lo("KIS", dong), bn)
    assert kq.hang_dau_tien == 3
...
def test_qa_dung_ca_frame_LAN_answer():
    bn = BangNhan([_nhan(task_type="QA", answer_text="màu xanh", answer_dung=True)])
    lo = _lo("QA", [DongNop("L01_V001", (505,), "màu xanh")])
    assert cham_query(lo, bn).r_scores[0] == 1.0
...
def test_qa_frame_dung_ma_ANSWER_SAI_thi_0():
    ...
    bn = BangNhan([
        _nhan(task_type="QA"),
        _nhan(task_type="QA", answer_text="màu trắng", answer_dung=False, label="wrong"),
    ])
    lo = _lo("QA", [DongNop("L01_V001", (505,), "màu trắng")])
    assert cham_query(lo, bn).r_scores[0] == 0.0
...
def test_qa_answer_CHUA_AI_CHAM_thi_bao_ra_chu_khong_nuot():
    ...
    """
    bn = BangNhan([_nhan(task_type="QA")])
    lo = _lo("QA", [DongNop("L01_V001", (505,), "chưa ai phán câu này")])
    kq = cham_query(lo, bn)
    assert kq.r_scores[0] == 0.0
    assert kq.answer_chua_cham == 1
...
def test_qa_frame_sai_thi_khong_tinh_la_answer_chua_cham():
    ...
    bn = BangNhan([_nhan(task_type="QA")])
    lo = _lo("QA", [DongNop("L01_V001", (999,), "gì đó")])
    assert cham_query(lo, bn).answer_chua_cham == 0
...
def test_trake_vi_du_BTC_ra_dung_075(bn_trake):
    ...
    """
    dong = DongNop("L10_V010", (101, 156, 203, 251))
...
def test_trake_khop_THEO_VI_TRI_khong_phai_bat_ky_thu_tu_nao(bn_trake):
    ...
    """
    dong = DongNop("L10_V010", (251, 203, 156, 101))
...
def test_trake_SAI_VIDEO_thi_0_du_moi_frame_deu_khop(bn_trake):
    ...
    dong = DongNop("L99_V999", (101, 150, 203, 251))
    assert r_score_trake(dong, "t1", bn_trake) == 0.0
...
def test_trake_diem_le_KHONG_co_trong_bang_rut_gon(bn_trake):
    ...
    """
    lo = _lo("TRAKE", [
        DongNop("L10_V010", (101, 150, 250, 260)),   # khớp 2/4 = 0.5
        DongNop("L99_V999", (0, 0, 0, 0)),
        DongNop("L10_V010", (101, 156, 203, 251)),   # khớp 3/4 = 0.75
    ], qid="t1")
    kq = cham_query(lo, bn_trake)
...
def test_trake_moment_idx_khong_de_dong_nay_de_len_dong_kia(bn_trake):
    ...
    assert bn_trake.so_khoanh_khac("t1") == 4
...
def test_query_CHUA_CO_NHAN_thi_bo_qua_chu_khong_tinh_0():
    ...
    """
    bn = BangNhan([_nhan(query_id="q1")])
    bc = cham_lo([_lo("KIS", [_kis()], qid="q1"),
                  _lo("KIS", [_kis()], qid="q_chua_cham")], bn)
    assert [k.query_id for k in bc.ket_qua] == ["q1"]
    assert bc.bo_qua == ["q_chua_cham"]
    assert bc.trung_binh()["final"] == pytest.approx(1.0), "câu chưa chấm đã kéo điểm xuống"


def test_tach_theo_dang_bai():
    """Gộp ba dạng vào một số là giấu mất chỗ đang hỏng."""
    bn = BangNhan([_nhan(query_id="k1"), _nhan(query_id="a1", task_type="QA")])
    bc = cham_lo([_lo("KIS", [_kis()], qid="k1"), _lo("QA", [_kis()], qid="a1")], bn)
    assert len(bc.theo_dang("KIS")) == 1 and len(bc.theo_dang("QA")) == 1
    assert bc.trung_binh(bc.theo_dang("KIS"))["final"] == pytest.approx(1.0)
    assert bc.trung_binh(bc.theo_dang("QA"))["final"] == 0.0  # answer chưa chấm
...
def test_nop_du_hon_100_dong_thi_cat():
    ...
    kq = cham_query(lo, bn)
    assert kq.n_dong == 100 and kq.final == 0.0
...
def test_doc_runs_dong_hong_thi_bo_qua_chu_khong_sap(tmp_path):
    ...
    p.write_text('{"hong": true}\n{"query_id":"q1","task_type":"KIS","answers":[]}\n',
                 encoding="utf-8")
    assert len(doc_runs(p)) == 1


def test_thieu_file_runs_thi_bao_ro(tmp_path):
    with pytest.raises(FileNotFoundError):
        doc_runs(tmp_path / "khong_co.jsonl")
...
def test_bao_cao_neu_ro_so_answer_chua_cham():
    bn = BangNhan([_nhan(task_type="QA")])
    bc = cham_lo([_lo("QA", [DongNop("L01_V001", (505,), "chưa phán")], qid="q1")], bn)
    assert "CHƯA AI CHẤM" in in_bao_cao(bc)


def test_bao_cao_liet_ke_cau_te_nhat_kem_ly_do():
    bn = BangNhan([_nhan(query_id="q1")])
    bc = cham_lo([_lo("KIS", [_kis(frame=600)], qid="q1")], bn)
    ra = in_bao_cao(bc)
    assert "TỆ NHẤT" in ra and "không có dòng đúng nào" in ra


def test_bao_cao_khong_sap_khi_khong_co_gi_de_cham():
    assert isinstance(in_bao_cao(cham_lo([], BangNhan([]))), str)
