...
# Chính trình thông dịch đang chạy test, KHÔNG phải đường dẫn venv đoán trước.
# Bản gốc ghi cứng `.venv/Scripts/python.exe` (layout Windows của máy Thạch) nên
# test chết ngay khi thu thập trên macOS/Linux — mà cái chết đó lại nằm ở
# subprocess nên đọc log tưởng script hỏng chứ không phải test hỏng.
...
def _fixture(tmp_path: Path) -> tuple[Path, Path, Path]:
    ...
    # Hỏi resolver chuẩn của dự án thay vì tự ghép đường dẫn. Bản gốc ghép tay
    # `data/raw/btc/keyframes/L30_V043/029.jpg` và thiếu đúng một cấp thư mục
    # (`keyframes_L30/`) mà kho BTC thật có — chính lớp lỗi mà `frame_assets`
    # sinh ra để dẹp (AGENTS.md: resolver là invariant dùng chung).
    resolution = resolve_frame_path("L30_V043", 1818, keyframe_id="L30_V043#k0029")
    if resolution.path is None or not resolution.path.is_file():
        pytest.skip(f"chưa có ảnh keyframe BTC trên máy này: {resolution.reason}")
    return queries, out_dir, resolution.path
